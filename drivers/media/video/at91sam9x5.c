...
static u32 at91sam9x5_video_handle_irqstat(struct at91sam9x5_video_priv *priv)
{
	...
	if (priv->hwstate == at91sam9x5_video_HW_IDLE &&
		...
		if (priv->cfgstate == at91sam9x5_video_CFG_BAD &&
				!(at91sam9x5_video_read32(priv, REG_HEOCHSR) & REG_HEOCHSR_CHSR))
			hwrunning = 0;

		if (!hwrunning || !at91sam9x5_video_buf_in_use(priv, &priv->cur)) {
			vb2_buffer_done(priv->cur.vb, VB2_BUF_STATE_DONE);	
			priv->cur = priv->next;
			priv->next.vb = NULL;
		}
...
static irqreturn_t at91sam9x5_video_irq(int irq, void *data)
{
	...
	if (handled & at91sam9x5_video_read32(priv, REG_HEOIMR)) {
		return IRQ_HANDLED;
	} else
		return IRQ_NONE;
}
		...
		int overlay_only)
{
	...
	if (priv->cfgupdate || overlay_only) {
		...
		if (rect->width <= 0 || rect->height <= 0 ||
			...
			if (overlay_only && priv->hwstate == at91sam9x5_video_HW_RUNNING) {
				if (priv->cfgstate == at91sam9x5_video_CFG_BAD) {
					priv->cfgstate = at91sam9x5_video_CFG_GOOD_LATCH;
					priv->hwstate = at91sam9x5_video_HW_IDLE;

					at91sam9x5_video_show_buf(priv, priv->cur.vb);
				} else
					at91sam9x5_video_write32(priv, REG_HEOCHER, REG_HEOCHER_UPDATEEN);
			} else
				priv->cfgstate = at91sam9x5_video_CFG_GOOD_LATCH;
		}
...
static int at91sam9x5_video_vb_buf_prepare(struct vb2_buffer *vb)
{
	...
	/* XXX: format-dependant */
	if (vb->v4l2_planes[0].length < pix->width * pix->height + ALIGN(pix->width, 2) * ALIGN(pix->height, 2) / 2 + 10 * 32)
		return -EINVAL;
